...
def training(features, target):
    try:
        model = repository.get(SGDClassifier(loss='hinge', penalty='l1'), SGDClassifier.__name__, DIR_PATH)
        scaler = repository.get(MinMaxScaler(), MinMaxScaler.__name__, DIR_PATH)
