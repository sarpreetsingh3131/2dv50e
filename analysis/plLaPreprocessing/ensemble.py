...
def main():
    ...
    for trainF, trainT, testF, testT in zip(trainSetsFs, trainSetsT, testSetsFs, testSetsT):
        ...
        for i in range(len(targets)//NB_ADAPTION_PER_CYCLE):
            for learner in ensLearner.estimators:
                learner.partial_fit(
                    trainF[i*NB_ADAPTION_PER_CYCLE:(i+1)*NB_ADAPTION_PER_CYCLE],
                    trainT[i*NB_ADAPTION_PER_CYCLE:(i+1)*NB_ADAPTION_PER_CYCLE]
                    )
